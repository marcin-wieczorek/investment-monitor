...
class DudaParser {
    ...
    private fun extractStatus(box: Element): pl.marcinwieczorek.investmentmonitor.domain.InvestmentStatus? {
        val sign = box.selectFirst("div.investment-sold-sign") ?: return null
        return when {
            sign.hasClass("status-w-sprzedazy") -> pl.marcinwieczorek.investmentmonitor.domain.InvestmentStatus.FOR_SALE
            sign.hasClass("status-sprzedaz-zakonczona") -> pl.marcinwieczorek.investmentmonitor.domain.InvestmentStatus.SOLD_OUT
            else -> pl.marcinwieczorek.investmentmonitor.domain.InvestmentStatus.UNKNOWN
        }
    }
